...
const Wordle = ({ length, attempts }: { length: number | 0; attempts: number | null }) => {
  ...
  }, [guesses, targetWord, newGame, attempts]);

  // Verificar os eventos do teclado, assim evito ter que verificar diretamente no input
  const keyboardHandler = async (e: React.KeyboardEvent<HTMLInputElement>) => {
  ...
  /*
  Função para colocar as cores nos blocos do jogo:
  - Cinzento: letra não pertence
  ...
  */
  ...
  // Mensagem de carregamento, enquanto não encontra a palavra da API
        ...
        {guesses.map((guess, rowIndex) => (
          <div key={rowIndex} className="flex gap-2">
            {guess.split("").map((letter, index) => (
              <div
                key={index}
                className={`w-12 h-12 flex items-center justify-center text-xl font-bold uppercase text-white border-2 border-gray-800 ${setBlockColor(
                  letter,
                  index
                )}`}
              >
